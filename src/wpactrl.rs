...
fn nix_err_to_io_err(err: nix::Error) -> Error {
    match err {
    ...
    pub fn recv(&mut self) -> Result<Option<String>> {
        if self.pending()? {
            ...
            std::str::from_utf8(&self.buffer[0..buf_len])
                ...
                .map_err(|_| Error::new(ErrorKind::InvalidData, "interpreting bytes as string"))
        } else {
            Ok(None)
        }
    }
    ...
    fn request<F: FnMut(&str)>(&mut self, cmd: &str, mut cb: F) -> Result<String> {
        ...
        loop {
            ...
            match self.handle.recv(&mut self.buffer) {
                Ok(len) => {
                    let s = std::str::from_utf8(&self.buffer[0..len]).map_err(|_| {
                        Error::new(ErrorKind::InvalidData, "interpreting bytes as string")
                    })?;
                    if s.starts_with('<') {
                        cb(s)
                    } else {
...
impl WpaCtrl {
    ...
    pub fn attach(mut self) -> Result<WpaCtrlAttached> {
        ...
        if self.0.request("ATTACH", |_: &str| ())? != "OK\n" {
            Err(Error::new(ErrorKind::InvalidData, "got invalid response"))
        } else {
